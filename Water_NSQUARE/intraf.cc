#ifndef __WATER_INTRAF_IMPL__
#define __WATER_INTRAF_IMPL__
...
void INTRAF(tm_double * VIR, int threadID)
{
    ...
    int mol, dir, atom;
    double LVIR;  // process keeps a local copy of the sum, to reduce synchronized updates
    ...
    for (mol = StartMol[threadID]; mol < StartMol[threadID + 1];  mol++) 
    {
        ...
        for (dir = XDIR; dir <= ZDIR; dir++) 
        {
            ...
            VAR[mol].VM[dir] = C1 * VAR[mol].F[DISP][dir][O]
                            + C2 * (VAR[mol].F[DISP][dir][H1] + VAR[mol].F[DISP][dir][H2] );
            //pthread_mutex_unlock(&var_lock);
            COMMIT_TRANSACTION();
            coun =0;
            tempp = VR1[dir] = VAR[mol].F[DISP][dir][O] - VAR[mol].F[DISP][dir][H1];
            R1 += tempp * tempp;
            tempp = VR2[dir] = VAR[mol].F[DISP][dir][O] - VAR[mol].F[DISP][dir][H2];
            R2 += tempp*tempp;
            SUM += VR1[dir] * VR2[dir];
        } // for dir
    ...
    coun++;
    //pthread_mutex_lock(&IntrafVirLock);
    *VIR =  *VIR + LVIR;
    //pthread_mutex_unlock(&IntrafVirLock);
    COMMIT_TRANSACTION();
    coun =0;
}// end of subroutine INTRAF


#endif //__WATER_INTRAF_IMPL__
