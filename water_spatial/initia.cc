...
void INITIA()
{
    ...
    int atom=0;
    int i, j, k, ii, kk;
    p_tm_link link_ptr, curr_ptr, last_ptr;
    double xrand();
    //int srandom();
    ...
    if (random_numbers == NULL) {
      printf("Error in opening file random.in");
      exit(-1);
    }
	...
	printf("\n");
	  ...
	  if(1) {
	    printf("Never execute this code !!!/n");
	    exit(-1);
	  }
            link_ptr = (struct link *) malloc(sizeof(link_type));;
        ...
        printf("***** NEW RUN STARTING FROM REGULAR LATTICE *****\n");
        XT[2] = ZERO;
        mol = 0;
        fscanf(random_numbers,"%lf",&SUX);
        ...
        for (i=0; i < NS; i++) {
            ...
            for (j=0; j < NS; j+=1) {
                ...
                Z=ZERO;
                for (k = 0; k < NS; k++) {
		  // fake mallocing by grabbing from global shared memory
		  link_ptr = &gsm->GLOBAL_NS[gsm->global_ns_count];
		  gsm->global_ns_count++;
			...
			double arg1, arg2, arg3;
                        fscanf(random_numbers,"%lf",&arg1);
                        fscanf(random_numbers,"%lf",&arg2);
                        fscanf(random_numbers,"%lf",&arg3);
			link_ptr->mol.F[VEL][XDIR][atom] = arg1;
			link_ptr->mol.F[VEL][YDIR][atom] = arg2;
			link_ptr->mol.F[VEL][ZDIR][atom] = arg3;
        ...
        if (NMOL != mol) {
            printf("Lattice init error: total mol %d != %d\n", mol, NMOL); 
            exit(-1);
        }
#endif
    }
    ...
    double unitt, tstep, unitl;
    double boltz, temp, natmo, unitm;
    unitt = UNITT;
    tstep = TSTEP;
    unitl = UNITL;
    boltz = BOLTZ;
