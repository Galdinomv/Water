...
/*                      they are out *\/ */
/*   unsigned ProcID; */
void BNDRY(unsigned ProcID)
{
    int i, j, k, dir; 
    int X_INDEX, Y_INDEX, Z_INDEX;
    p_tm_link curr_ptr, last_ptr, next_ptr, temp_ptr;
	...
	//printf("k = %d\n",k);
		...
		INIT_TRANSACTIONS();
		BEGIN_TRANSACTION();
	      //pthread_mutex_lock(&mol_lock[6+i+j+k]); ////////
		printf("ergrrfhdfhds\n"); fflush(stdout);
              if (last_ptr != NULL) 
		...
		printf("EKJGEKGKGK\n"); fflush(stdout);
	      //pthread_mutex_unlock(&mol_lock[6+i+j+k]);
		COMMIT_TRANSACTION();

                /* Add link to BOX[X_INDEX][Y_INDEX][Z_INDEX] */
                
		//pthread_mutex_lock(&mol_lock[6+X_INDEX+Y_INDEX+Z_INDEX]); //////// LOCK
	        //INIT_TRANSACTIONS();
		BEGIN_TRANSACTION();
                temp_ptr = BOX[X_INDEX][Y_INDEX][Z_INDEX].list;
                BOX[X_INDEX][Y_INDEX][Z_INDEX].list = curr_ptr;

                curr_ptr->next_mol = temp_ptr;///////////////// lock
		COMMIT_TRANSACTION();
        	//pthread_mutex_unlock(&mol_lock[6+X_INDEX+Y_INDEX+Z_INDEX]);
            }
