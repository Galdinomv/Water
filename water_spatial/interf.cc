...
/*   double *VIR; */
/*   unsigned ProcID; */
void UPDATE_FORCES(p_tm_link link_ptr, int DEST, double XL[], double YL[], double ZL[], double FF[]);
 
void INTERF(int DEST, unsigned threadID) 
{
    ...
    double  FTEMP;
    double LVIR = 0.0;
    p_tm_link curr_ptr, neighbor_ptr;
    struct list_of_boxes *curr_box;
    double *temp_p;
    
    //printf("%d accessing my_boxes in interf\n",threadID);
    curr_box = my_boxes[threadID];
    //printf("%d using curr_box as %d\n",threadID,curr_box);
    //printf("%d got my_boxes in interf\n",threadID);
    while (curr_box) {
        ...
        for (XBOX=i-1; XBOX<=i+1; XBOX++) {
		   ...
		   // cout << threadID << " getting neighbor_ptr" << endl;
		    //cout << threadID << " BOX_PER_SIDE = " << BOX_PER_SIDE << endl;
		    //cout << threadID << " XNUM = " << X_NUM << " Y_NUM = " << Y_NUM << " Z_NUM = " << Z_NUM << endl;
                    neighbor_ptr = BOX[X_NUM][Y_NUM][Z_NUM].list;
		    //cout << threadID << " got neighbor_ptr" << endl;		    
                    
                    while (neighbor_ptr) {
                        
                        /* go through current box list */
		      //cout << threadID << " getting cur_ptr ijk" << endl;
		      curr_ptr = BOX[i][j][k].list;
		      //cout << threadID << " got cur_ptr ijk" << endl;		                             
                        while (curr_ptr) {
			    ...
			    //cout << threadID << " calling CSHIFT" << endl;
                            ...
                            //cout << threadID << " done calling CSHIFT" << endl;
